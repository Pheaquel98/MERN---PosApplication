...
const Products = ({ categories, filtered, products, setProducts, search }) => {
  ...
  return (
    <div className="product-wrapper grid gap-4 grid-cols-card">
      {filtered
        .filter((product) => product.title.toLowerCase().includes(search)) // arattığımız ürünün herhangi bir harfini içeriyorsa döner
        .map((item) => (
      ...
      <div
        className="product-item border hover:shadow-lg cursor-pointer transition-all select-none bg-orange-600 flex justify-center items-center hover:opacity-90 min-h-[180px]"
